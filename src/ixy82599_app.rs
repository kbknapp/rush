...
impl engine::App for Ixy82599App {
    ...
    fn report(&self) {
        ...
        println!(
            "  Device stats for {} since last report:",
            ixy.get_pci_addr()
        );
        println!(
            "     rxpackets:\t{:10}",
            lib::comma_value(stats.rx_pkts - last_rx_pkts)
        );
        println!(
            "     rxbytes:\t{:10}",
            lib::comma_value(stats.rx_bytes - last_rx_bytes)
        );
        println!(
            "     txpackets:\t{:10}",
            lib::comma_value(stats.tx_pkts - last_tx_pkts)
        );
        println!(
            "     txbytes:\t{:10}",
            lib::comma_value(stats.tx_bytes - last_tx_bytes)
        );
    }
...
mod selftest {
    ...
    #[test]
    fn ixy_send_recv() {
        let nic0 = if let Ok(pci) = std::env::var("RUSH_INTEL10G0") {
            pci
        } else {
            println!("Skipping test (need RUSH_INTEL10G0)");
            return;
        };
        let nic1 = if let Ok(pci) = std::env::var("RUSH_INTEL10G1") {
            pci
        } else {
            println!("Skipping test (need RUSH_INTEL10G1)");
            return;
        };
